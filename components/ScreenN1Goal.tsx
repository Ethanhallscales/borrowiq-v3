...
import type { NextPropertyGoal } from "@/lib/types";

interface Props {
  step: number;
  totalSteps: number;
  onComplete: (g: NextPropertyGoal) => void;
  onBack: () => void;
}

const GOALS: { id: NextPropertyGoal; label: string; sub: string; icon: string }[] = [
  { id: "live-in",    label: "A Home to Live In",      sub: "Your primary residence",                      icon: "🏠" },
  { id: "investment", label: "An Investment Property",  sub: "Build your portfolio, generate rental income", icon: "📈" },
];

export default function ScreenN1Goal({ step, totalSteps, onComplete, onBack }: Props) {
  const [selected, setSelected]   = useState<NextPropertyGoal | null>(null);
  const [advancing, setAdvancing] = useState(false);

  function handle(id: NextPropertyGoal) {
    if (advancing) return;
    setSelected(id);
    setAdvancing(true);
              ...
              <motion.button key={g.id} type="button" onClick={() => handle(g.id)} disabled={advancing}
                ...
                <motion.div
                  ...
                  <div className="flex-1">
                    <motion.p
                      style={{ fontFamily: "var(--font-bebas-neue)", fontSize: "1.4rem", letterSpacing: "0.05em" }}
