...
import type { QuizData, BuyingSituation, EmploymentType } from "./types";
...
export function shadeIncome(rawIncome: number, employment: EmploymentType): number {
  switch (employment) {
    case "payg":          return rawIncome;
    case "self-employed": return rawIncome * 0.8;
    case "contractor":    return rawIncome * 0.8;
    case "casual":        return rawIncome * 0.7;
    default:              return rawIncome;
  }
}
...
export function calculatePathA(quiz: QuizData): ResultsA {
  ...
  // Shaded primary income (silently applied — not shown to user)
  ...
  const combinedGross  = shadedPrimary + shadedPartner;

  const { netIncome: primaryNet } = calculateTax(shadedPrimary);
  const { netIncome: partnerNet  } = calculateTax(shadedPartner);
  const totalNetAnnual = primaryNet + partnerNet;
  const monthlyNet     = totalNetAnnual / 12;

  // Monthly commitments
  const hecsAnnual  = hecsDebt > 0 ? calculateHecsRepayment(shadedPrimary) : 0;
...
export function calculatePathB(quiz: QuizData): ResultsB {
  ...
  const { netIncome: pNet } = calculateTax(shadedPrimary + shadedPartner);
...
export function computeAdditionalBorrowingN(
  ...
  const combinedGross = shadedPrimary + shadedPartner + shadedRentalAnnual;
...
export function calculatePathN(quiz: QuizData): ResultsN {
  ...
  const shadedPrimary = shadeIncome(annualIncome, employmentType);
  const shadedPartner = buyingSituation === "partner" ? shadeIncome(partnerIncome, "payg") : 0;
  const grossIncome   = shadedPrimary + shadedPartner;
